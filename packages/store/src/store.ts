...
class TypeOpsUtils {
  static handleAppendPrependList(input: { newState: any; ai: DataAndTypeOps }) {
    ...
    if (ai.optimisticFailed) {
      ...
      // newState = { ...newState, [name]: ai.data }; TODO name
      ...
      if (discard) {
        // newState = { ...newState, [name]: {} }; // Note: Currently we only support types ops on Fetch fields so we can assume that value is object , name global name conflict
      } else {
        // newState = { ...newState, [name]: ai.data }; // TODO name global name conflict
      }
    } else {
      // newState = { ...newState, [name]: ai.data }; // TODO global name conflict
    }
    return newState;
  }
  ...
  static handleUpdateList(input: {
    ...
    if (ai.optimisticFailed) {
      ...
      if (discard) {
        // newState = { ...newState, [name]: {} }; // Note: Currently we only support types ops on Fetch fields so we can assume that value is object , global name conflict
      } else {
        // newState = { ...newState, [name]: ai.data }; // Global name conflict
      }
    } else {
      // newState = { ...newState, [name]: ai.data }; // Global name conflict
    }
