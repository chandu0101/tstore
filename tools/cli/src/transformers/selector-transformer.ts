...
type DependenciesOfObject = {
  parent: string;
  values: { value: string; childDeps?: DependenciesOfObject }[];
};
 ...
 * @param pa
 */
const getPropertyAccessvalueAndItsParent = (
  pa: ts.Identifier
): { value: string; pn?: ts.Node } => {
  ...
  const innerFn = (node: ts.Node): any => {
    ...
    if (ts.isPropertyAccessExpression(node)) {
      ...
      // if element access expression probably identifier we can not process further
      return;
    } else if (ts.isVariableDeclaration(node)) {
      if (isNodeFurtherTracable(node)) {
        console.log(
          "****** selector transformer got variable decl ",
          node.getText()
        );
        pn = node;
      }
...
const getDepencyObjectForCallExpression = (
  ce: ts.CallExpression,
  parent: string
): DependenciesOfObject => {
  ...
  return getDependenciesOfObject(identiFier, {
    variableUsage,
    parentObj: parent,
  });
};

const getDepenciesOfIdentifier = (
  id: ts.Identifier,
  variableUsage: Map<ts.Identifier, VariableInfo>
) => {
  let result: { value: string; childDeps?: DependenciesOfObject }[] = [];
  const usage = variableUsage.get(id);
  if (usage) {
    const valueuses = usage.uses.filter(
      (vu) => vu.domain === UsageDomain.ValueOrNamespace
    );
    valueuses.forEach((vu) => {
      const loc = vu.location;
      console.log(
        "***** getDepenciesOfIdentifier ",
        loc.getText(),
        loc.parent.getText()
      );
...
const getDependenciesOfObjectBindingpattern = (
  obp: ts.ObjectBindingPattern,
  options: GetDependenciesOfObjectOptions
) => {
  let result: { value: string; childDeps?: DependenciesOfObject }[] = [];
  obp.elements.forEach((e) => {
    ...
    result.push({
      value: v,
      childDeps: getDependenciesOfObject(e.name, { ...options, parentObj: v }),
    });
  });
  return result;
};

type GetDependenciesOfObjectOptions = {
  parentObj?: string;
  variableUsage: Map<ts.Identifier, VariableInfo>;
};
 ...
 * @param obj
 */
const getDependenciesOfObject = (
  node: ts.BindingName,
  options: GetDependenciesOfObjectOptions
): DependenciesOfObject => {
  const result: DependenciesOfObject = {
    values: [],
    parent: options.parentObj ? options.parentObj : "state",
  };
...
const processChildDependecy = (cdo?: DependenciesOfObject): string => {
  if (!cdo) return "";
  return cdo.values
    .map((v) => `${v.value}.${processChildDependecy(v.childDeps)}`)
    .join(".");
};
...
const processDependencyObject2 = (
  dg: DependenciesOfObject
): Record<string, O1> => {
...
const getSelectorFromFunction = (
  f: ts.ArrowFunction | ts.FunctionExpression,
  name: string,
  ext: boolean
) => {
  if (!f.type) {
  ...
  if (ext && f.parameters.length < 2) {
    throw new Error(
      "createSelectorE should be a function state and external input "
    );
  }
  ...
  let result = `export const ${name}:Selector<${sType},${rType}> = {fn:${f.getText()},dependencies:${JSON.stringify(
    pg
  )}}`;
  if (ext) {
    const extInputType = f.parameters[1].type!.getText();
    result = `export const ${name}:SelectorE<${sType},${extInputType},${rType}> = {fne:${f.getText()},dependencies:${JSON.stringify(
      pg
    )}}`;
  }
  return result;
...
const createSelctorSelectorNode = (
  ...
  if (ts.isArrowFunction(arg) || ts.isFunctionExpression(arg)) {
    ...
    throw new Error(
      `You should specify array function or normal function for createSelector`
    );
  }
  ...
  if (ms === "@typesafe-store/store") {
    if (id.importClause && id.importClause.namedBindings) {
      ...
      if (ts.isNamedImports(nBindings)) {
        result = Boolean(
          nBindings.elements.find((nb) => nb.getText() === "Selector")
        );
      }
    }
  }
...
const selectorTransformer: ts.TransformerFactory<ts.Node> = (context) => {
...
const transformFile = (file: string) => {
  try {
    ...
    const transformedContent = printer.printFile(newSf as any); // TODO newSF sourcefile
    let imports: string[] = [];
