...
import { ActionMeta, } from "@typesafe-store/store";
...
function transformFile(file: string) {
    try {
        ...
        console.info(chalk.red(`Error processing file : ${relative(".", file)} : ${error} stack : ${error.stack}`))
    }
...
const createReducerFunction = (cd: ts.ClassDeclaration) => {
    ...
    let result = ts.factory.createIdentifier("")
    if (methodsResults.length === 0 && asyncActionType === "") { // If no async properties and methods then return empty node
        result = ts.factory.createIdentifier(EMPTY_REDUCER_TRANFORM_MESSAGE)
    } else {
        ...
        result = ts.factory.createIdentifier(
            `
        ...
        .map(m => {
            ...
            const processForEachStatement = (s: ts.ExpressionStatement): ForEachStatementResult => {
                ...
                if (Array.isArray(forachStatement)) {
                    result.statementResults = processStatements(forachStatement as any, { isReturnSupported: true }) // TODO check cast to any
                } else {
            ...
            const processTerinaryStatement = (s: ts.ExpressionStatement): TerinaryStatementResult => {
                ...
                result.statement = s
                result.trueExp = processMutationStatement(ts.factory.createExpressionStatement(cond.whenTrue))
                result.falseExp = processMutationStatement(ts.factory.createExpressionStatement(cond.whenFalse))
                return result
            }
...
const processFetchProp2 = (lpd: LocalPropertyDecls, group: string): ProcessFetchResult => {
    ...
    return {
        name, actionPayload, responseType: "", grpcMeta, graphql, offload,
        tf: transformFunctionQueryNode?.exprName.getText(), typeOps: typeOpsResult
    }
    ...
    const asyncType = propDecls
        ...
        .map(p => {
            ...
            if (tpe.includes("_ts_pmeta") || tpe.startsWith("TSPromiseFieldValue")) {
                ...
                // result = `{name:"${name
                //     }",group:"${group}", fetch: ${fr.actionPayload}  }`;  //TODO
...
export const createPropertyAccessForString = (
    ...
    if (input.length == 2) {
        const f = input[0];
        const n = f === "this" ? ts.factory.createThis : ts.factory.createIdentifier(f);
        return ts.factory.createPropertyAccessExpression(n as ts.Expression, ts.factory.createIdentifier(input[1]));
    }
    ...
    if (!onlyThisInput) {
        values.forEach(v => {
            ...
            if (isUnionType(type)) {
                ...
                const ul = utype.types[0];
                // const ul = utype.types[0] // TODO cross check its returning  union memebers in reverse order :s 
                (type as ts.UnionType).types.forEach(m => {
...
function processThisStatementOffload(exp: ts.Node, options: ProcessStatementsOptions = {}): ProcessThisResult {
    ...
    if (!onlyThisInput) {
        values.reverse().some(v => {
            let type = getTypeForPropertyAccess(v.name.split("."))
            const nonNullType = AstUtils.getNonNullableType(type)
            const tpeStr = AstUtils.typeToString(nonNullType).trim()
            let isOptionalType = false
            if (isUnionType(type)) {
                type = type as ts.UnionType;
                const ul = (type as ts.UnionType).types[0] // TODO cross check its returning  union memebers in reverse order :s 
